...
function calcularFaixa2(s) { if(s<=2259.20)return 0; return(Math.min(s,2826.65)-2259.20)*0.075; }
function calcularFaixa3(s) { if(s<=2826.65)return 0; return(Math.min(s,3751.05)-2826.65)*0.15; }
function calcularFaixa4(s) { if(s<=3751.05)return 0; return(Math.min(s,4664.68)-3751.05)*0.225; }
function calcularFaixa5(s) { if(s<=4664.68)return 0; return(s-4664.68)*0.275; }
function calcularImpostoDue(s) { return calcularFaixa1(s)+calcularFaixa2(s)+calcularFaixa3(s)+calcularFaixa4(s)+calcularFaixa5(s); }
function calcularAliquotaEfetiva(salario,ir) { if(salario===0)return 0; return(ir/salario)*100; }

function fmtMoeda(v) { return v.toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2}); }
function fmtPct(v) { return v.toLocaleString('pt-BR',{minimumFractionDigits:2,maximumFractionDigits:2})+'%'; }
function parseSal(txt) { return parseFloat(txt.trim().replace(/\./g,'').replace(',','.')); }
...
function calcular() {
  ...
  setTimeout(function(){
    btn.classList.remove('loading'); btn.setAttribute('aria-busy','false');
    var s=parseSal(inp.value);
    if(isNaN(s)||s<=0){ err.classList.add('vis'); inp.style.borderColor='#d32f2f'; inp.setAttribute('aria-invalid','true'); inp.focus(); return; }
    err.classList.remove('vis'); inp.style.borderColor=''; inp.setAttribute('aria-invalid','false');
    var ir=calcularImpostoDue(s); var alq=calcularAliquotaEfetiva(s,ir);
    ...
    var res=document.getElementById('resultado'); res.classList.add('vis');
    dispararConfete();
...
function preencherTabela(s) {
  var tbody=document.getElementById('tbodyRes'); tbody.innerHTML='';
  var vals=[calcularFaixa1(s),calcularFaixa2(s),calcularFaixa3(s),calcularFaixa4(s),calcularFaixa5(s)];
  var fAtiva=0;
  for(var i=0;i<FAIXAS.length;i++){ if(s>(i>0?FAIXAS[i-1].limite:0))fAtiva=i; }
  ...
  FAIXAS.forEach(function(f,i){
    ...
    tr.innerHTML='<td><span class="pill '+f.pill+'">'+f.texto+'</span>'+marc+'</td>'+
      '<td class="mono col-h">'+lims[i]+'</td>'+
      '<td class="dir" style="color:'+(vals[i]>0?'var(--color-brand-blue-dark)':'var(--color-text-muted)')+';">'+(vals[i]>0?'R$ '+fmtMoeda(vals[i]):'—')+'</td>';
    tbody.appendChild(tr);
  });
  var tot=vals.reduce(function(acc,v){return acc+v;},0);
  var tt=document.createElement('tr');
  tt.style.fontWeight='700'; tt.style.borderTop='2px solid var(--borda)';
  tt.innerHTML='<td colspan="2">Total do Imposto</td><td class="dir" style="color:var(--color-brand-green-dark);">R$ '+fmtMoeda(tot)+'</td>';
...
function gerarPDF() {
  ...
  doc.text('DADOS DE ENTRADA',M,y); doc.setDrawColor(0,102,51); doc.setLineWidth(.3); doc.line(M,y+2,W-M,y+2); y+=10;
  doc.setTextColor(18,33,58); doc.setFont('helvetica','normal'); doc.setFontSize(10);
  doc.text('Salário Mensal Bruto:',M,y); doc.setFont('helvetica','bold'); doc.text('R$ '+fmtMoeda(s),90,y); y+=14;
  ...
  doc.setTextColor(90,106,126); doc.setFontSize(7.5); doc.setFont('helvetica','normal'); doc.text('IMPOSTO A RECOLHER',M+4,y+8);
  doc.setTextColor(0,102,51); doc.setFontSize(14); doc.setFont('helvetica','bold'); doc.text(irT,M+4,y+18);
  doc.setFillColor(216,229,245); doc.roundedRect(112,y,80,22,3,3,'F');
  doc.setTextColor(90,106,126); doc.setFontSize(7.5); doc.setFont('helvetica','normal'); doc.text('ALÍQUOTA EFETIVA',116,y+8);
  doc.setTextColor(0,48,130); doc.setFontSize(14); doc.setFont('helvetica','bold'); doc.text(alT,116,y+18); y+=30;
  doc.setFont('helvetica','bold'); doc.setFontSize(9); doc.setTextColor(0,102,51);
  ...
  doc.text('Faixa',M+3,y+5.5); doc.text('Alíquota',68,y+5.5); doc.text('Limite',96,y+5.5); doc.text('Imposto na Faixa',190,y+5.5,{align:'right'}); y+=8;
  var vals=[calcularFaixa1(s),calcularFaixa2(s),calcularFaixa3(s),calcularFaixa4(s),calcularFaixa5(s)];
  ...
  FAIXAS.forEach(function(f,i){
    doc.setFillColor(i%2===0?248:241,i%2===0?250:245,i%2===0?252:250); doc.rect(M,y,W-2*M,8,'F');
    ...
    doc.setFont('helvetica','bold'); doc.text(vals[i]>0?'R$ '+fmtMoeda(vals[i]):'—',190,y+5.5,{align:'right'}); y+=8;
  });
...
function toggleAcord(header) {
  ...
  document.querySelectorAll('.acord-header.aberto').forEach(function(h){
    h.classList.remove('aberto'); h.setAttribute('aria-expanded','false');
    h.nextElementSibling.classList.remove('aberto');
  });
  if(!aberto){ header.classList.add('aberto'); header.setAttribute('aria-expanded','true'); body.classList.add('aberto'); }
}
...
document.querySelectorAll('.acord-header').forEach(function(h){
  h.addEventListener('keydown',function(e){
    if(e.key==='Enter'||e.key===' '){ e.preventDefault(); toggleAcord(h); }
  });
});
...
function dispararConfete(){
  cvs=document.getElementById('confete'); ctx2=cvs.getContext('2d');
  cvs.width=window.innerWidth; cvs.height=window.innerHeight; cvs.style.display='block'; parts=[];
  for(var i=0;i<130;i++){
    parts.push({x:Math.random()*cvs.width,y:Math.random()*-cvs.height,sz:Math.random()*10+5,
      cor:CORES[Math.floor(Math.random()*CORES.length)],sh:FORMAS[Math.floor(Math.random()*FORMAS.length)],
      vy:Math.random()*3+2,vx:Math.random()*2-1,rot:Math.random()*360,vr:Math.random()*6-3});
  }
  if(rafId)cancelAnimationFrame(rafId);
  loopConfete(); setTimeout(pararConfete,3600);
}

function loopConfete(){
  ctx2.clearRect(0,0,cvs.width,cvs.height); var fora=true;
...
function mostrarCarimbo(){
  var el=document.getElementById('carimbo'); el.classList.add('ativo');
  setTimeout(function(){
...
(function(){
  var TEXTO='Digite seu salário mensal';
  var VEL_DIGITAR=90, VEL_APAGAR=60, PAUSA_PISCAR=60000, PAUSA_RESET=800;
  var el=document.getElementById('typingEl');
  if(!el)return;
  function digitar(i){
    if(i>TEXTO.length){ setTimeout(function(){ apagar(TEXTO.length); },PAUSA_PISCAR); return; }
    el.textContent=TEXTO.slice(0,i);
    setTimeout(function(){ digitar(i+1); },VEL_DIGITAR);
  }
  function apagar(i){
    if(i<0){ setTimeout(function(){ digitar(0); },PAUSA_RESET); return; }
    el.textContent=TEXTO.slice(0,i);
    setTimeout(function(){ apagar(i-1); },VEL_APAGAR);
  }
  setTimeout(function(){ digitar(0); },600);
})();
